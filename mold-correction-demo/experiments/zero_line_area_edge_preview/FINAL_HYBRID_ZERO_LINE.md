...
- 2번 경로형 제로라인: 박준혁 원본 1~6단계 전체 로직
- 두 로직의 입력 형식 변환과 공통 추가 조건: 내부 어댑터

`park_junhyeok_original/`에는 박준혁의 라벨 제거, 외곽 그래프, 제로점 선정,
`±0.7mm` 보정영역, 병합 소스를 원본 그대로 보존합니다.
`park_junhyeok_route_selector.py`도 박준혁의 `select_nearest_zero_points.py`
원본을 수정하지 않고 보존합니다. `park_junhyeok_adapter.py`가 이 단계들을

"""Select JD_67XX zero-point pairs from the latest my_lab-style candidates."""
...
def project_zero_points_to_outer_contour(
    zero_points: list[dict],
    contour_points: np.ndarray,
) -> None:
    """Map my_lab scan-loop points to the outer-contour traversal order.

    The step-03 sample indices are local to three separate nested scan loops,
    so they cannot be compared directly with the dense outer-contour indices
    used by this step. Projection preserves the original point coordinates
    while supplying a consistent clockwise/counter-clockwise ordering key.
    """
    for point in zero_points:
        coordinate = np.asarray(point["point"], dtype=np.float64)
        squared_distances = np.sum((contour_points - coordinate) ** 2, axis=1)
        projected_index = int(np.argmin(squared_distances))
        point["source_loop_sample_index"] = point["sample_index"]
        point["sample_index"] = projected_index
        point["distance_to_outer_contour_px"] = float(
            np.sqrt(squared_distances[projected_index])
        )
...
def select_along_outer_contour(
    ...
    for logical_info in logical_infos:
        ...
        for _rank_sum, _max_rank, reverse_rank, forward_rank, reverse_point, forward_point in ranked_pairs:
            ...
            if validation["valid"]:
                selected_pair = (reverse_point, forward_point, reverse_rank, forward_rank, validation)
                break
        if selected_pair is None:
            raise ValueError(f"{region['label']}: no zero-point pair forms a valid closed enclosure")

        reverse_point, forward_point, reverse_rank, forward_rank, closure_validation = selected_pair
        ...
        selections.append(
            {
                ...
                "selection_status": "closed_enclosure_validated",
                "directional_candidate_ranks": [reverse_rank + 1, forward_rank + 1],
...
def draw_result(
    ...
    cv2.putText(canvas, "retained correction regions (+/-1.0 mm)", (legend_left + 55, 48), cv2.FONT_HERSHEY_SIMPLEX, 0.39, (30, 30, 30), 1, cv2.LINE_AA)
    ...
    cv2.putText(canvas, "first zero point in each contour direction", (legend_left + 55, 101), cv2.FONT_HERSHEY_SIMPLEX, 0.39, (30, 30, 30), 1, cv2.LINE_AA)
...
def process(
    ...
    project_zero_points_to_outer_contour(zero_points, outer_contour["points"])
    ...
    result = {
        ...
        "selection_rule": {
            "method": "Follow the product outer contour from the correction-region contact arc in both directions and select the first zero point encountered in each direction",
            "selected_per_region": 2,
            "candidate_zero_points": "all 18 my_lab-style scan-loop zero points from step 03, projected only for outer-contour traversal ordering",
            ...
            "invalid_pair_fallback": "advance to the next zero point in contour-traversal order until a valid closed enclosure is found",

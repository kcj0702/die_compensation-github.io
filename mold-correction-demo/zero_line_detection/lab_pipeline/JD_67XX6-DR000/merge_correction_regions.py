"""Merge nearby JD_67XX +/-1.0 correction areas and remove small regions."""
...
def render_overlay(
    ...
    cv2.putText(
        result,
        f"merged large correction regions (+/-{TOLERANCE_MM:.1f} mm)",
        (x0 + 54, 48),
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=f"Merge nearby +/-{TOLERANCE_MM:.1f} correction regions and remove small regions"
    )
    parser.add_argument(
        "--correction-mask",
        type=Path,
        default=product_root / "output" / "04_out_of_tolerance" / f"outside_pm{tolerance_label}_mask.png",
    )
